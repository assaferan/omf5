...
cmds = [["./src/omf5 -genus=data/qf5db.sage -isom -disc=%d -hecke -p=%d > data/hecke_mat_%d_%d.dat 2> logs/hecke_mat_%d_%d.log &" %(d, p, d, p, d, p) for p in ps[d] ] for d in discs]
...
cmds = [["./src/omf5 -genus=data/qf5db.sage -isom -disc=%d -hecke -p=%d -row > data/hecke_row_%d_%d.dat 2> logs/hecke_row_%d_%d.log &" %(d, p, d, p, d, p) for p in ps[d] ] for d in discs]
cmds = reduce(lambda x,y : x+y, cmds)
for cmd in cmds:
    print "Executing %s" %(cmd)
    os.system(cmd)
