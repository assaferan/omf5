...
int process_isotropic_vector_nbr_data(nbr_data_t nbr_man, int* T, const hash_table_t genus, slong spinor_prime,
				      double* theta_time, double* isom_time, double* total_time, int* num_isom)
{
  ...
  cputime = clock();
  i = hash_table_indexof(genus, nbr, 0, theta_time, isom_time, num_isom);
...
int process_isotropic_vector(neighbor_manager* nbr_man, int* T, const hash_table_t genus,
			     double* theta_time, double* isom_time, double* total_time, int* num_isom)
...
int process_neighbour_chunk(int* T, int p, int i, int gen_idx, const hash_table_t genus,
			    double* theta_time, double* isom_time, double* total_time, int* num_isom)
{
  matrix_TYP *Q;
...
int process_neighbour_chunk_nbr_data(int* T, int p, int k, int gen_idx, const hash_table_t genus, slong spinor_prime,
				     double* theta_time, double* isom_time, double* total_time, int* num_isom)
{
  matrix_TYP *Q;
  nbr_data_t nbr_man;
...
void hecke_col_nbr_data(int* T, int p, int k, int gen_idx, const hash_table_t genus, slong spinor_prime)
{
  int num_isom, lc;
...
void hecke_col(int* T, int p, int gen_idx, const hash_table_t genus)
{
  int num;
  int num_isom, lc;
  double theta_time, isom_time, total_time;
...
matrix_TYP* hecke_matrix(const hash_table_t genus, int p)
{
  matrix_TYP* hecke;
  int gen_idx;
...
void get_hecke_ev_nbr_data(nf_elem_t e, const hash_table_t genus, eigenvalues* evs,
...
void get_hecke_ev(nf_elem_t e, const hash_table_t genus, eigenvalues* evs, int p, int ev_idx)
{
  int* a;
  int num, i, pivot;
  nf_elem_t prod;
...
void get_hecke_fmpq_mat(fmpq_mat_t hecke_fmpq_mat, const hash_table_t genus, int p)
{
...
bool decomposition_finite_subspace(decomposition* decomp, const hash_table_t genus, const fmpq_mat_t basis_V,
...
bool decomposition_finite(decomposition* decomp, const hash_table_t genus, const int* ps, slong num_ps)
{
  fmpq_mat_t basis_M;
  slong dim;
  bool is_complete;
...
decomposition* decompose(const hash_table_t genus)
{
  slong bound, num_ps;
  int* ps;
...
eigenvalues* hecke_eigenforms(const hash_table_t genus)
{
  fmpq_mat_t T;
  slong i;
  eigenvalues* evs;
